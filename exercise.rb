...
new_hash= hash[:response][:results]
new_hash.each do |article|
  article[:views] = 0
end



def read_article(new_hash)

random_article = new_hash.sample
random_article[:views] += 1
puts random_article
end

read_article(new_hash)

def display_views(new_hash)
